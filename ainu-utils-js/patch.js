// Borrowed from GitHub comment by @loynoir
// https://github.com/rustwasm/wasm-pack/issues/1334#issuecomment-1783654073
...
const content = await readFile(`./pkg/${name}.js`, "utf8");
...
const bytes = __toBinary(${JSON.stringify(
      await readFile(`./pkg/${name}_bg.wasm`, "base64")
    )});
`
  );

await writeFile(`./pkg/${name}.js`, patched);
